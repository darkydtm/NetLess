...
class MeshRuntimeTest {
	...
	fun `duplicate packet replays terminal receipt without handing off content twice`() = runTest {
		...
		assertEquals(receipt, network.destination.receive(packet, TransportType.WifiDirect), "replayed receipt")
...
private class ThreeNodeNetwork(failDestination: Boolean = false) {
	...
	init {
		...
		fun runtime(node: NodeId, store: RelayStore?, transports: List<Pair<TransportType, NodeId>>, onContent: suspend (ContentEnvelope) -> Unit = {}) = MeshRuntime(
			...
			localIdentity = keys.getValue(node.value), signSession = { signSession(keys.getValue(node.value), it) }, verifySession = { key, data, signature -> keys.values.any { it.encoded.contentEquals(key.encoded) && signSession(it, data) == signature } }, nowMillis = { 1_000L }
		)
	...
	fun restartDestination() {
		...
		destination = MeshRuntime(
			...
			verifySession = { key, data, signature -> keys.values.any { it.encoded.contentEquals(key.encoded) && signSession(it, data) == signature } },
			nowMillis = { 1_000L },
		)
	}
					...
					when (val decoded = ControlCodec.decode(response)) {
						...
						is Receipt -> when (peer.value) {
							"destination" -> {
								...
								if (decoded.value.state != DeliveryState.Delivered) network.relayFailureReceipt = decoded.value
							}
